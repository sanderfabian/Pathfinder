...
const AddMDP = () => {
    ...
    const [majorTitle] = useState('Masters');
    ...
    const createSubcollections = async (programId, majorDocumentName) => {
        try {
            ...
            await Promise.all(subcollections.map(async (subcollection) => {
                const subcollectionRef = collection(firestore, 'MasterDegree', programId, 'Major', majorDocumentName, subcollection);
    ...
    const createProgramDocument = async () => {
        try {
            ...
            const majorCollection = collection(firestore, 'MasterDegree', programId, 'Major');
            const majorDocument = {
                Title: majorTitle,
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={6}>
                            <TextField
                                ...
                                variant="outlined"
                                value={majorTitle}
                                // Disable the text field for majorTitle
                                disabled
                                fullWidth
                                style={{ marginBottom: '1rem' }}
                            />
