...
const MDMajorDetails = () => {
    ...
    useEffect(() => {
        const fetchMajorDetails = async () => {
            try {
                ...
                const majors = majorSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), programId }));
    ...
    const handleEdit = (id, rowData) => {
        navigate(`/md-edit-major/${programId}/${id}`, { state: { majorDetails: rowData } });
    };

    const handleViewCourses = (id) => {
        navigate(`/view-md-courses/${programId}/${id}`);
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            await Promise.all(subcollections.map(async (subcollection) => {
                const subcollectionRef = collection(firestore, 'MasterDegree', programId, 'Major', id, subcollection);
                const snapshot = await getDocs(subcollectionRef);
                snapshot.forEach(async (doc) => {
                    await deleteDoc(doc.ref);
                });
            }));
            await deleteDoc(doc(firestore, 'MasterDegree', programId, 'Major', id));
            setMajorData((prevMajorData) => prevMajorData.filter((major) => major.id !== id));
            console.log('Major document and subcollections deleted successfully');
        } catch (error) {
            console.error('Error deleting major document and subcollections:', error);
        }
    };
