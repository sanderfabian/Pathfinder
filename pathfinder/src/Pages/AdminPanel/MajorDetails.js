...
const MajorDetails = () => {
    const { programId, degreeType } = useParams();
    ...
    useEffect(() => {
        const fetchMajorDetails = async () => {
            try {
                const majorCollectionRef = collection(firestore, 'BachelorDegree', programId, 'Major');
    ...
    const columns: GridColDef[] = [
        { field: 'id', headerName: <div style={{ fontWeight: 'bold' }}>ID</div>, width: 150 }, // Ensure there is an 'id' field
    ...
    const handleEdit = (id) => {
        navigate(`/edit-major/${programId}/${id}`);
    };
    const handleViewCourses = (id) => {
        navigate(`/view-courses/${programId}/${id}`);
    };

    const handleDelete = async (id) => {
        try {
            const majorDocRef = doc(firestore, 'BachelorDegree', programId, 'Major', id);
            const subcollections = ['CoreCourses', 'ElectiveCourses', 'ElectiveMajorCourses', 'MajorCourses'];  // Reference to subcollections inside the Major collection
            await Promise.all(subcollections.map(async (subcollection) => {
                const subcollectionRef = collection(firestore, 'BachelorDegree', programId, 'Major', id, subcollection);
            ...
            await deleteDoc(majorDocRef); // Delete the major document
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={12}>
                            <DataGrid
                                ...
                                editable={{
                                    onRowEditCommit: async (updatedRow) => {
                                        try {
                                            const docRef = doc(firestore, 'BachelorDegree', programId, 'Major', updatedRow.id);
                                            await setDoc(docRef, updatedRow);
                                            setMajorData((prevMajorData) =>
                                                prevMajorData.map((major) => (major.id === updatedRow.id ? { ...major, ...updatedRow } : major))
                                            );
