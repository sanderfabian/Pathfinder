    ...
    const fetchSubCollections = async (cleanedDegreeValue, majorValue, study) => {
        try {
            ...
            while (compulsoryCoursesData.length > 0) {
                let course = compulsoryCoursesData.shift();
                ...
                const meetsUnitRequirement = course.RequiredUnit ? totalUnitsCompleted >= course.RequiredUnit : true;

                // Add the course if it meets RequiredUnit and all its prerequisites are already sorted
                if (meetsUnitRequirement && (!hasPrerequisites || hasNilPrerequisite || course.CompulsoryPrerequisite.every(prerequisite => sortedCoursesData.find(c => c.id === prerequisite)))) {
                    sortedCoursesData.push(course);
                    totalUnitsCompleted += course.Unit; // Add the unit of the added course to totalUnitsCompleted
                } else {
                    compulsoryCoursesData.push(course); // Put back the course if it doesn't meet conditions
                }
            }
